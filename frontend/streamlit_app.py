    ...
    with col2:
        ...
        with st.container():
            if st.session_state.auth_mode == "register":
                ...
                if submitted:
                    if len(full_name.strip()) < 2: st.error("Enter your full name.")
                    elif "@" not in email: st.error("Enter a valid email address.")
                        ...
                        if ok:
                            st.success("Account created. Please sign in."); st.session_state.auth_mode = "login"
                        else: st.error(data)
            else:
                st.header("Log in")
                ...
                if submitted:
                    if not email or not password: st.error("Email and password are required.")
                    else:
...
def require_auth():
    ...
    if ok:
        st.session_state.user = data; return True
    st.warning(data); return False


def home():
    st.title("Home Dashboard")
    ok_c, clients = api_request("GET", "/clients"); ok_k, contracts = api_request("GET", "/contracts")
    c1,c2,c3 = st.columns(3)
    c1.metric("Clients", len(clients) if ok_c else 0); c2.metric("Contracts", len(contracts) if ok_k else 0); c3.metric("Review status", "Ready")
...
def clients_page():
    ...
    with st.form("client_form", clear_on_submit=True):
        name = st.text_input("Client name"); industry = st.text_input("Industry"); notes = st.text_area("Notes")
        if st.form_submit_button("Create client"):
            ok, data = api_request("POST", "/clients", json={"name": name, "industry": industry, "notes": notes})
            st.success("Client created.") if ok else st.error(data)
    ok, data = api_request("GET", "/clients")
    if ok and data: st.dataframe(data, use_container_width=True)
    else: st.caption("No clients yet.")
...
def contracts_page():
    ...
    with st.form("upload_form"):
        ...
        if st.form_submit_button("Upload contract"):
            if not file: st.error("Choose a contract file.")
            else:
                files = {"file": (file.name, file.getvalue())}; data = {"name": name or file.name}
                if client_name != "No client": data["client_id"] = client_options[client_name]
                ok, resp = api_request("POST", "/contracts/upload", files=files, data=data)
                st.success("Contract uploaded.") if ok else st.error(resp)
    ...
    else: st.caption("No contracts uploaded yet.")


def select_contract():
    ok, contracts = api_request("GET", "/contracts")
    if not ok or not contracts:
        st.warning("Upload a contract first."); return None
...
def analysis_page():
    ...
    if cid and st.button("Run analysis", use_container_width=True):
        ...
        if ok:
            st.metric("Health score", data["health_score"]); st.write(data["executive_summary"]); st.subheader("Risks"); st.write(data["risks"]); st.subheader("Clauses"); st.write(data["clauses"])
        else: st.error(data)


def chat_page():
    st.title("Contract Chat")
    cid = select_contract(); q = st.text_input("Ask a question grounded in this contract")
    if cid and st.button("Ask"):
        ok, data = api_request("POST", f"/contracts/{cid}/chat", json={"question": q})
        if ok: st.write(data["answer"]); st.caption(f"Confidence: {data['confidence']}"); st.write(data["evidence"])
        else: st.error(data)
...
def benchmark_page():
    ...
    if cid and st.button("Run benchmark", use_container_width=True):
        ...
        if ok: st.metric("Overall score", data["overall_score"]); st.write(data)
        else: st.error(data)
